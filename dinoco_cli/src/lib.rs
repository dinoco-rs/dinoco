...
enum DatabaseCommands {
    ...
    #[command(about = "Introspect the database and generate dinoco/schema.dinoco")]
    Introspect {},
}
...
pub async fn run() {
    ...
    match &cli.command {
        ...
        Commands::Database(command) => match command {
            ...
            &DatabaseCommands::Introspect {} => {
                if let Err(err) = introspect_database().await {
                    eprintln!("❌ Failed to introspect database.");
                    eprintln!("👉 Details: {}", err);

                    eprintln!("\n💡 Possible causes:");
                    eprintln!("- Database connection failure");
                    eprintln!("- Missing DATABASE_URL environment variable");
                    eprintln!("- Insufficient permissions to read metadata");

                    eprintln!("\n🛠️ Suggestions:");
                    eprintln!("- Verify DATABASE_URL or schema connection config");
                    eprintln!("- Ensure the database is accessible");
                    eprintln!("- Confirm metadata/query permissions");
                }
            }
        },
