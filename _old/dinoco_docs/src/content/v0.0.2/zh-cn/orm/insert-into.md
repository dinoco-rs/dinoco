...
当您希望 `.values(...)` 接受包含新关系和现有连接的更丰富负载时，请将 `#[insertable]` 与 `#[derive(Extend)]` 一起使用。Dinoco 会首先插入父级，然后递归处理所有内容。
...
使用带有 `#[insertable]` 的 Extend 负载来创建父级，并在同一流程中创建相关项。
...
当关系指向已存在的记录时，代码生成器会生成一个像 `ArticleConnection` 这样的枚举，用于负载中。
...
- [**派生**](/v0.0.2/core/derives)：了解何时使用 `Rowable`、`Extend` 和 `#[insertable]`。
- [**`insert_many::&lt;M&gt;()`**](/v0.0.2/orm/insert-many)：批量插入。
- [**`update::&lt;M&gt;()`**](/v0.0.2/orm/update)：更新记录。
