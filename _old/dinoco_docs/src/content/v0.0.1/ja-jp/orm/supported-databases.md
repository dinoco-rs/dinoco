...
**Dinoco** は、**PostgreSQL**、**MySQL**、**SQLite** データベースをネイティブにサポートしています。
近日中に新しいデータベースへのサポートを拡大する予定です！

> ⚠️
> 現在、Dinoco は **直接接続 URL** のみをサポートしています。
...
設定は、スキーマファイルの `config` ブロック内で行われます。

### SQLite

SQLite を使用する場合、データベースファイルは `dinoco` フォルダ内に自動的に生成されます。
