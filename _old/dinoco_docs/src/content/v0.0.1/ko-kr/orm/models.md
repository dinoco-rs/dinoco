...
모델은 Dinoco 스키마에서 애플리케이션의 핵심 엔터티를 정의합니다. 각 모델은 일반적으로 데이터베이스의 테이블을 나타내며 코드 생성, 유형화된 쿼리 및 Dinoco API 작업을 위한 기반 역할을 합니다.
...
스키마에서 더 친숙한 모델 이름을 유지하면서 데이터베이스의 다른 물리적 이름으로 매핑하려는 경우 `@@table_name()`을 사용합니다.
...
코드 생성 후 이 모델은 Dinoco API와 직접 함께 사용할 수 있습니다.
...
| 목록 필드   | `tags String[]`        | 다중 값 저장    |
| ID          | `id Integer @id`       | 고유하게 식별   |
| 기본값      | `@default(now())`      | 자동으로 채우기 |
