...
Die `Modelle` definieren die zentralen Entitäten Ihrer Anwendung im Dinoco-Schema. Jedes `Modell` repräsentiert normalerweise eine Tabelle in der Datenbank und dient als Grundlage für Codegenerierung, typisierte Abfragen und Operationen mit der Dinoco-API.
...
Felder können grundlegende Schemwerte wie Text, Zahlen, Booleans und Daten darstellen.

### Skalare Felder

Dies sind Felder, die direkte Werte wie Text, Zahlen, Booleans und Daten speichern.
...
`name` kann null oder fehlend sein, abhängig von der Datenbank und der generierten Schicht.
...
Neben Attributen in einzelnen Feldern unterstützt Dinoco auch Decorators, die auf den gesamten Modellblock angewendet werden.
...
Dieses Format ist nützlich in Assoziationstabellen und Szenarien, in denen die natürliche Eindeutigkeit bereits zusammengesetzt ist.

### `@@table_name("...")`

Verwenden Sie `@@table_name()`, wenn Sie einen benutzerfreundlicheren Modellnamen im Schema beibehalten, aber auf einen anderen physischen Namen in der Datenbank abbilden möchten.
...
Wenn Sie atomare Updates in einem einzigen Feld wünschen, ist der `find_and_update`-Workflow oft noch direkter:
...
| :---------------- | :--------------------- | :-------------------------- |
| Modell            | `model User { ... }`   | Eine Entität repräsentieren |
...
| Eindeutig         | `email String @unique` | Duplikate vermeiden         |
...
- In der Datenbank persistiert zu werden.
- Eine eigene Identität zu haben.
- Isoliert abgefragt zu werden.
- Eigene Lese- und Schreibregeln zu haben.
