...
if (baseVersion === undefined) {
	throw new Error('No localized versions data found for v0.0.4.');
}
...
const versionData: VersionData = {
	description: Object.fromEntries(localizedVersions.flatMap(localeVersion => (localeVersion.description === undefined ? [] : [[localeVersion.locale, localeVersion.description] as const]))),
	groups: baseVersion.groups.map((group, index) => ({
		icon: group.icon,
		languages: Object.fromEntries(localizedVersions.map(localeVersion => [localeVersion.locale, mergeSections(group.sections, localeVersion.groups[index]?.sections)])),
		name: group.name,
