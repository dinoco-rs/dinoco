...
impl DinocoGenericRow for Row {
    fn get_value(&self, idx: usize) -> DinocoResult<DinocoValue> {
        let val = self
            .as_ref(idx)
            .ok_or_else(|| DinocoError::ParseError(format!("Column {} not found", idx)))?;
        ...
        match val {
            ...
            MyValue::Date(year, month, day, hour, min, sec, micros) => {
                ...
                let time =
                    NaiveTime::from_hms_micro_opt(*hour as u32, *min as u32, *sec as u32, *micros)
            ...
            MyValue::Time(_, _, _, _, _, _) => Err(DinocoError::ParseError(
                "MySQL TIME cannot be mapped to DateTime<Utc>".into(),
            )),
        }
    }
}
