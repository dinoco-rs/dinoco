...
const NavigationCard = ({ align, href, label, title }: { align: 'left' | 'right'; href: string; label: string; title: string }) => {
	...
	return (
		<Link
			href={href}
			className={clsx(
				'group flex min-h-[112px] flex-1 flex-col justify-between rounded-lg px-5 py-4 not-dark:shadow-md not-dark:border not-dark:border-gray-200',
...
const DocsContentNavigation: React.FC<DocsContentNavigationProps> = ({ locale, previous, next }) => {
	if (!previous && !next) {
		return null;
	}
	const { navigation } = getIntlMessages(locale);

	return (
		<nav aria-label={navigation.title} className="mt-10 grid gap-4 border-t border-light-200 pt-6 dark:border-dark-700 md:grid-cols-2">
			{previous ? <NavigationCard label={navigation.previous} title={previous.item.name} href={previous.path} align="left" /> : <div className="hidden md:block" aria-hidden="true" />}

			{next ? <NavigationCard label={navigation.next} title={next.item.name} href={next.path} align="right" /> : <div className="hidden md:block" aria-hidden="true" />}
		</nav>
	);
};
