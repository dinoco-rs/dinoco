...
const CodeBlockPre = ({ children, className, ...props }: MarkdownComponentProps): React.JSX.Element => {
	...
	const [locale, setLocale] = useState<DocsLocale>('en-us');
	...
	const { code: labels } = getIntlMessages(locale);

	useEffect(() => {
		setLocale(resolveDocsLocale(document.documentElement.lang));
	}, []);
	...
	return (
		<div
			...
			className={['mb-6 overflow-hidden rounded-lg border border-light-300 bg-light-50 shadow-sm dark:border-dark-700 dark:bg-dark-900', className].filter(Boolean).join(' ')}
		>
			<div className="flex items-center justify-between border-b border-light-300 bg-light-100 px-4 py-2.5 dark:border-dark-700 dark:bg-dark-950">
				<p className="text-xs font-semibold uppercase text-slate-500 dark:text-slate-400">{getLanguageLabel(language)}</p>
