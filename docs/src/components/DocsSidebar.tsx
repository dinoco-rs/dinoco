				...
				{hasSubItems ? (
					<div className={clsx('flex items-center border-l -ml-[1px] text-sm transition-colors', isItemActive || isChildActive ? 'border-dinoco-brand dark:border-dinoco-cyan' : 'border-transparent hover:border-slate-300 dark:hover:border-slate-600')}>
						...
						<button
							...
							aria-label={isOpen ? navigation.closeSubItems : navigation.openSubItems}
						>
		...
		<>
			...
			>
				<div className="mb-5 flex items-center gap-2 lg:hidden">
					<button type="button" onClick={onClose} aria-label={intl.closeMenuLabel} title={intl.closeMenuLabel} className="cursor-pointer rounded-md p-2 text-slate-500 hover:bg-light-200 dark:text-slate-400 dark:hover:bg-[#161616]">
						<FiX size={20} />
					</button>
				...
				<nav className="space-y-8 pt-6">
					{localizedSections.map(section => (
						<div key={section.title}>
							<h4 className="mb-3 text-xs font-bold uppercase text-slate-900 dark:text-white">{section.title}</h4>
