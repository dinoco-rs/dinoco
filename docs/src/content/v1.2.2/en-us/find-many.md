...
Repeated `where_` calls are combined with `AND`. Use [Where complex](/v1.2.1/orm/where-complex) when precedence requires explicit `AND`, `OR`, and `NOT` groups.
...
See [Full-text search](/v1.2.1/orm/full-text-search) for adapter differences.
...
The return becomes `Vec<AccountSummary>`. `includes(...)` can load relations, filter children, and paginate per parent. See [Select](/v1.2.1/orm/select) and [Includes](/v1.2.1/orm/includes).
