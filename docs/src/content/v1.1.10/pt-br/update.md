...
Os mesmos campos funcionam em `update`, `update_many` e `find_and_update`, inclusive com `.returning::<T>()` nos dois primeiros. Eles também funcionam dentro de batches transacionais: o write do endpoint e as alterações na pivô recebem commit ou rollback juntos, e o builder continua produzindo uma única posição de resultado.
...
Consulte [Many-to-many implícito](/v1.1.10/guide/relations#many-to-many-implícito) para a API completa pelos endpoints e as regras da tabela pivô.

Para atualizar exatamente uma row e receber a entity completa sem `.returning()`, consulte a página dedicada [Find and update](/v1.1.10/orm/find-and-update).
