...
A busca seleciona a row que será atualizada e retornada. Veja [Find and update](/v1.1.10/orm/find-and-update).
...
`find_first` e `find_many` com full-text podem ser adicionados normalmente:

```rust
let batch = dinoco::transaction![
    dinoco::find_first::<Account>()
        .where_(|account| account.name.fulltext("matheus")),
    dinoco::find_many::<Account>()
        .where_(|account| account.biography.fulltext("rust")),
];

let results = dinoco::transactions(batch)
    .execute(&client)
    .await?;
```

O adapter compila a mesma condição dentro da conexão transacional.
...
- Ranking, idioma configurável, frases e tokenizers customizados não fazem parte da v1.1.10.
