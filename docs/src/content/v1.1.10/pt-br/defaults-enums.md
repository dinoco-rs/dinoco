...
O enum gerado também implementa `Display`, então `.to_string()` retorna exatamente o valor do schema, em vez do nome PascalCase da variante Rust. `TryFrom<&str>`, `TryFrom<String>` e `FromStr` fazem a conversão inversa e retornam erro para valores desconhecidos. Por exemplo, `waiting_payment` vira `PaymentState::WaitingPayment`; `.to_string()` retorna `"waiting_payment"`, e `PaymentState::try_from("waiting_payment")` reconstrói a mesma variante.
