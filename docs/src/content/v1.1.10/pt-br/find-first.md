...
Para grupos explícitos, use [Where complex](/v1.1.10/orm/where-complex). Fields `@fulltext` também funcionam:
...
Consulte [Select](/v1.1.10/orm/select) e [Includes](/v1.1.10/orm/includes).
...
Todos os includes dessa consulta seguem o primary.

## Use em uma transaction

`find_first` pode ser adicionado a `Transaction` e mantém seus filtros, seleção e `where_complex`. Includes dentro da batch ainda não são suportados.
