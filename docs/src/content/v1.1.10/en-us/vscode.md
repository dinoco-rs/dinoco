...
The v1.1.10 language server provides:

- syntax and semantic highlighting;
- live parser and schema diagnostics;
- context-aware completion for config keys, scalars, models, enums, field attributes such as `@index` and `@fulltext`, model attributes such as `@@ids`, `@@uniques`, `@@indexes`, and `@@fulltexts`, fields inside their arrays, defaults, relation keys, and referential actions;
- hover documentation;
- go to definition, references, and safe rename, including field references inside model attributes;
...
Remote and virtual workspaces are not supported in v1.1.10 because generated files, local commands, and database prompts require a real workspace filesystem.
