...
The search selects the row to update and return. See [Find and update](/v1.1.10/orm/find-and-update).
...
Full-text `find_first` and `find_many` builders can be added normally:
...
The adapter compiles the same condition inside the transaction connection.
...
- Ranking, configurable language, phrases, and custom tokenizers are outside v1.1.10.
