...
Use [Where complex](/v1.1.10/orm/where-complex) for explicit groups. `@fulltext` fields work here too:
...
See [Select](/v1.1.10/orm/select) and [Includes](/v1.1.10/orm/includes).
...
Every include in that query follows the primary.

## Use in a transaction

`find_first` can be added to `Transaction` and preserves filters, selection, and `where_complex`. Includes inside a batch are not supported yet.
