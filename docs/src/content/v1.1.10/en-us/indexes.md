...
See [Full-text search](/v1.1.10/orm/full-text-search) for `.fulltext(...)` across every find builder.
...
The planner emits `CREATE INDEX`, `DROP INDEX`, or the adapter's full-text variant. Introspection compares index name, columns, order, and kind. Snapshots created before v1.1.10 remain compatible: a historically missing `indexes` property is not interpreted as an intentional removal.
