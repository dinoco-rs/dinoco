...
The virtual key is a write-only relation command and stays `None` on loaded entities. See [Implicit many-to-many](/v1.1.10/guide/relations#implicit-many-to-many) for both relation directions, disconnect, inserts, and pivot behavior.
...
When `where_complex` is present, every `where_` on the builder is ignored, before or after it. See [Where complex](/v1.1.10/orm/where-complex).
...
- It is unavailable in MySQL transactions in v1.1.10.
