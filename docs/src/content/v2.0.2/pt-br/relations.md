...
### Conectar/desconectar vários endpoints de uma vez

`.connect(...)`/`.disconnect(...)` preparam um ID de destino por chamada. Quando você precisa ligar (ou desligar) vários destinos em um único round trip, use `.connect_batch(...)`/`.disconnect_batch(...)` em vez de encadear `.connect(...)` repetidamente:

```rust
dinoco::update::<Business>()
    .where_(|business| business.id.eq(&business_id))
    .update(|business| business.system_id.connect_batch(vec![system_a.id.clone(), system_b.id.clone(), system_c.id.clone()]))
    .execute(&client)
    .await?;
```

Isso ainda emite um único `INSERT` na tabela de pivô — uma linha por destino conectado — em vez de um `INSERT` por chamada de `.connect(...)`. `.disconnect_batch(...)` é o espelho disso, colapsando em um único `DELETE ... WHERE ... IN (...)`:

```rust
dinoco::update::<Business>()
    .where_(|business| business.id.eq(&business_id))
    .update(|business| business.system_id.disconnect_batch(vec![system_a.id.clone(), system_b.id.clone()]))
    .execute(&client)
    .await?;
```

Ambos aceitam qualquer coisa iterável — um `Vec`, um array, um iterador — de valores conversíveis para o tipo de chave do destino. Estão disponíveis em qualquer lugar que `.connect(...)`/`.disconnect(...)` estejam (veja [Builders de update suportados](#builders-de-update-suportados) abaixo), e podem ser combinados livremente com chamadas de `.connect(...)`/`.disconnect(...)` em outros fields dentro do mesmo encadeamento `.update(...)`.

### Builders de update suportados

Chaves virtuais de many-to-many suportam `.connect(...)`/`.disconnect(...)` (e suas variantes `_batch`) em:
