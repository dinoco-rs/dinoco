...
`.pluck(...)` is a lighter alternative to `.returning::<S>()` when only one column is needed — it returns `Vec<T>` directly instead of building a full row model:
...
It replaces `.returning::<S>()` rather than combining with it, and is available on both `update` and `update_many`.

## Transform the returned rows

Chain `.transform(...)` after `.returning::<S>()` to map every returned row on the Rust side, after the `UPDATE` has already run:
...
Linking (or unlinking) several tags at once? `.connect_batch(...)`/`.disconnect_batch(...)` collapse the whole list into one `INSERT`/`DELETE` instead of one per tag — see [Connect/disconnect several endpoints at once](/en-us/docs/orm/orm/relations#connectdisconnect-several-endpoints-at-once).
