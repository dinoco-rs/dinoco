...
| Check whether any row matches | `exists::<M>()` | `bool` |
...
- `select::<S>()`;
- `pluck(...)` for a single column instead of a full row;
- `transform(...)` for a post-query Rust-side mapping;
- `includes(...)`;
- `order_by(...)`;
- `read_in_primary()`.
...
`exists::<M>()` compiles down to `SELECT EXISTS(...)` and returns a plain `bool`, without materializing any row at all:
...
It supports `where_`, `where_complex`, and `read_in_primary()`, same as `find_first`/`find_many`.

## Batch several queries together

`find_batch(...)` runs a tuple of independent `find_many`/`find_first` builders and hands back a matching tuple of their results:
...
Tuples of 2 to 8 items are supported, and items may mix `find_many` and `find_first`. `.includes(...)` isn't supported on items passed to `find_batch(...)` yet — build those relations with a plain `find_many`/`find_first` call instead.

How many round trips this takes to the database depends on the client's `QueryMode`:

- **`QueryMode::BatchQuery`** (the default) runs every item as its own query — identical to calling `.execute(...)` on each of them, just gathered into one `.await`.
- **`QueryMode::SingleQuery`** compiles every item into a JSON-aggregated subquery (`json_build_object`/`json_agg` on PostgreSQL, `JSON_OBJECT`/`JSON_ARRAYAGG` on MySQL, `json_object`/`json_group_array` on SQLite) and selects them all together in a single round trip.
...
or from `schema.dinoco`, so every generated `connect()` picks it up automatically — see [Configuration](/en-us/docs/orm/orm/configuration).
