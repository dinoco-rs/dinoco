...
`where_complex(...)` composes `AND`/`OR` groups exactly as it does on `find_many`. It replaces any earlier `where_(...)` conditions, and later `where_(...)` calls are ignored:

```rust
let count = dinoco::count::<User>()
    .where_complex(|x, c| c.or(x.active.eq(true), x.email.ends_with("@example.com")))
    .execute(&client)
    .await?;
```
