...
### Connect/disconnect several endpoints at once

`.connect(...)`/`.disconnect(...)` each stage one target ID per call. When you need to link (or unlink) several targets in one round trip, use `.connect_batch(...)`/`.disconnect_batch(...)` instead of chaining `.connect(...)` repeatedly:
...
This still issues a single `INSERT` into the pivot table — one row per connected target — instead of one `INSERT` per `.connect(...)` call. `.disconnect_batch(...)` is the mirror image, collapsing into one `DELETE ... WHERE ... IN (...)`:
...
Both accept anything iterable — a `Vec`, an array, an iterator — of values convertible into the target's key type. They're available anywhere `.connect(...)`/`.disconnect(...)` are (see [Supported update builders](#supported-update-builders) below), and can be mixed freely with `.connect(...)`/`.disconnect(...)` calls on other fields within the same `.update(...)` chain.

### Supported update builders

Virtual many-to-many keys support `.connect(...)`/`.disconnect(...)` (and their `_batch` counterparts) from:
