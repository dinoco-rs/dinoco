...
It replaces `.returning::<S>()` rather than combining with it, and is available on both `delete` and `delete_many`.

## Transform the returned rows

Chain `.transform(...)` after `.returning::<S>()` to map every deleted row on the Rust side, after the `DELETE` has already run:
