...
Enums, and the `mod` list of models, land in `dinoco/models/mod.rs`; each model gets its own file in `dinoco/models/`. Generated files are fully replaced on every regeneration, so never hand-edit them — the edit would be discarded on the next `migrate generate` or `models generate`.

To add derives, attributes, imports, or methods to what is generated, use [code transforms](/en-us/docs/orm/guide/transforms): a `dinoco/transform.rs` applied on every generation. The generated `dinoco/mod.rs` also starts with `#![allow(unused)]`, which only suppresses unused-code warnings within that module and the files it includes.
