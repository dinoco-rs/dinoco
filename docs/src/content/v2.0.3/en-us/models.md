...
The generated root module starts with `#![allow(unused)]`, which suppresses unused-code warnings from the generated module and the files it imports — it has no effect on your own application code. If you need extra derives, attributes, or methods on generated types, that belongs in a [code transform](/en-us/docs/orm/guide/transforms), not in a hand-edited generated file (which would just be overwritten).
