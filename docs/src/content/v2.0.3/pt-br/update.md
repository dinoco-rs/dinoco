...
`.pluck(...)` é uma alternativa mais leve a `.returning::<S>()` quando só uma coluna é necessária — retorna `Vec<T>` diretamente em vez de construir um row model completo:

```rust
let updated_ids = dinoco::update_many::<User>()
    .where_(|x| x.office.eq("support"))
    .update(|x| x.active.set(false))
    .pluck(|x| x.id)
    .execute(&client)
...
Ele substitui `.returning::<S>()` em vez de se combinar com ele, e está disponível tanto em `update` quanto em `update_many`.

## Transforme as rows retornadas

Encadeie `.transform(...)` depois de `.returning::<S>()` para mapear cada row retornada do lado do Rust, depois que o `UPDATE` já rodou:
...
let changed = dinoco::update::<User>()
    ...
    .update(|x| x.active.set(false))
    .returning::<UserSummary>()
    .transform(|user| serde_json::json!({ "id": user.id, "active": user.active }))
    .execute(&client)
...
Precisa ligar (ou desligar) várias tags de uma vez? `.connect_batch(...)`/`.disconnect_batch(...)` colapsam a lista inteira em um único `INSERT`/`DELETE` em vez de um por tag — veja [Conectar/desconectar vários endpoints de uma vez](/pt-br/docs/orm/guide/relations#conectardesconectar-vários-endpoints-de-uma-vez).
