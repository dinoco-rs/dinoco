...
```rust
let deleted_ids = dinoco::delete_many::<Session>()
    .where_(|x| x.expires_at.lt(cutoff))
    .pluck(|x| x.id)
    .execute(&client)
    .await?; // Vec<i64>
...
Ele substitui `.returning::<S>()` em vez de se combinar com ele, e está disponível tanto em `delete` quanto em `delete_many`.

## Transforme as rows retornadas

Encadeie `.transform(...)` depois de `.returning::<S>()` para mapear cada row removida do lado do Rust, depois que o `DELETE` já rodou:

```rust
let deleted = dinoco::delete::<User>()
    .where_(|x| x.id.eq(&user_id))
    .returning::<UserSummary>()
    .transform(|user| serde_json::json!({ "id": user.id, "email": user.email }))
    .execute(&client)
