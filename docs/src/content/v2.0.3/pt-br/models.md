...
O módulo raiz gerado começa com `#![allow(unused)]`, que suprime warnings de código não usado do módulo gerado e dos arquivos que ele importa — isso não tem efeito nenhum sobre o código da sua própria aplicação. Se você precisa de derives, atributos ou métodos extras nos tipos gerados, isso é papel de um [code transform](/pt-br/docs/orm/guide/transforms), não de editar um arquivo gerado à mão (que seria simplesmente sobrescrito).
