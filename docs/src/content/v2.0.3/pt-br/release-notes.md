...
- **`find_batch(...)`.** Roda uma tupla de 2 a 8 builders `find_many`/`find_first` independentes e retorna uma tupla correspondente de resultados. O novo `QueryMode` (`BatchQuery`, o default; ou `SingleQuery`, que combina cada item em um único round trip agregado em JSON) é configurável por client via `.with_query_mode(...)` ou pelo `schema.dinoco` via `config.query_mode`. Veja [Visão geral de queries](/pt-br/docs/orm/orm/find#rodando-várias-queries-juntas) e [Configuração](/pt-br/docs/orm/guide/configuration#modo-de-execução-do-find_batch).
