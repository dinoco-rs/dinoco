...
UUID keys are declared as `String` in the schema and Snowflake keys as `Integer`. Dinoco v1.1.2 follows the referenced field and preserves its generated Rust wrapper:
...
`insert_into` and `insert_many` with populated virtual IDs can also be placed in a transaction. Their endpoint and pivot rows are atomic. The endpoint ID must be known before execution through UUID, Snowflake, or a caller-provided value; a transactional insert cannot use a populated virtual relation key when its endpoint primary key is `autoincrement()`. See [Many-to-many relations in transactions](/v1.1.2/orm/transactions#many-to-many-relations) for complete examples and adapter limitations.
