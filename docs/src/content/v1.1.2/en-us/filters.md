...
Fields declared with `@fulltext` also expose `.fulltext(term)`. Regular strings do not have that method. See [Full-text search](/v1.1.2/orm/full-text-search).
...
For precedence with `AND`, `OR`, and `NOT`, see the dedicated [Where complex](/v1.1.2/orm/where-complex) page.
