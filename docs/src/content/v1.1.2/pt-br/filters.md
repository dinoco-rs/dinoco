...
Fields declarados com `@fulltext` também expõem `.fulltext(termo)`. Strings comuns não possuem esse method. Veja [Busca full-text](/v1.1.2/orm/full-text-search).
...
Para precedência com `AND`, `OR` e `NOT`, consulte a página dedicada [Where complex](/v1.1.2/orm/where-complex).
