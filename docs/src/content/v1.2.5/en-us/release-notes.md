...
The VS Code language server now treats only the main `dinoco/schema.dinoco` as the owner of project configuration. Imported schema files no longer report missing database configuration or `snowflake_node_id`, and formatting those files no longer invokes project-level validation. Named `import { ... } from "..."` statements also receive complete syntax highlighting.

Dinoco v1.2.5 adds database-side numeric updates, typed atomic-mutation and transaction errors, and the closure transaction API. `find_and_update` now supports `increment`, `decrement`, `multiply`, and `divide`; every chained field change is emitted in one conditional `UPDATE`, and no matching row returns `AtomicUpdateError::RowNotAffected`.
...
Child files can import direct declarations with `import { Model, Enum } from "relative/path.dinoco"`. The compiler resolves the complete import tree, normalizes relative paths, reports missing or circular dependencies at their source, and preserves the originating file and line in schema diagnostics.
