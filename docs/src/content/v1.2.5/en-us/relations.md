...
UUID keys are declared as `String` in the schema and Snowflake keys as `Integer`. Dinoco v1.2.5 follows the referenced field and preserves its generated Rust wrapper:
...
`insert_into` and `insert_many` with populated virtual IDs can also execute with `.execute(tx)` inside the closure transaction API, making their endpoint and pivot rows atomic. See [Transactions](/v1.2.5/orm/transactions#supported-builders) for the supported mutation flow.
