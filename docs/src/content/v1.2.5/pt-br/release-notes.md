...
O language server do VS Code agora considera apenas o `dinoco/schema.dinoco` principal como proprietário da configuração do projeto. Arquivos de schema importados não exibem mais erros de configuração do banco ou de `snowflake_node_id`, e a formatação desses arquivos não executa mais a validação global do projeto. Declarações nomeadas `import { ... } from "..."` também recebem syntax highlighting completo.

O Dinoco v1.2.5 adiciona updates numéricos executados pelo banco, erros tipados para mutations atômicas e transactions, além da nova API transacional baseada em closure. `find_and_update` agora suporta `increment`, `decrement`, `multiply` e `divide`; todas as alterações encadeadas viram um único `UPDATE` condicional, e a ausência de uma row compatível retorna `AtomicUpdateError::RowNotAffected`.
...
Arquivos filhos podem importar declarações diretas com `import { Model, Enum } from "caminho/relativo.dinoco"`. O compiler resolve toda a árvore de imports, normaliza caminhos relativos, informa dependências ausentes ou circulares em sua origem e preserva o arquivo e a linha original nos diagnósticos do schema.

`config.custom_derives` pode aplicar derives Rust adicionais globalmente aos enums ou structs de model gerados. Cada item declara seu alvo, caminho do derive e declaração `use`; o codegen deduplica derives e imports repetidos. Os arquivos `dinoco/mod.rs` gerados agora incluem `#![allow(dead_code)]`.
