...
Fields declared with `@fulltext` also expose `.fulltext(term)`. Regular strings do not have that method. See [Full-text search](/v1.2.1/orm/full-text-search).
...
For precedence with `AND`, `OR`, and `NOT`, see the dedicated [Where complex](/v1.2.1/orm/where-complex) page.
