...
Each variant also receives `#[serde(rename = "...")]`, so Serde serializes and deserializes the exact database value instead of the PascalCase Rust name. The generated enum implements `Display`, so `.to_string()` follows the same mapping. `TryFrom<&str>`, `TryFrom<String>`, and `FromStr` provide the reverse conversion and return an error for unknown values. For example, `waiting_payment` becomes `PaymentState::WaitingPayment`; `.to_string()` returns `"waiting_payment"`, and `PaymentState::try_from("waiting_payment")` reconstructs the same variant.
