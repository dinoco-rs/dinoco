...
O ID virtual é um comando write-only da relação e continua `None` nas entities carregadas. Consulte [Many-to-many implícito](/v1.2.0/guide/relations#many-to-many-implícito) para os dois sentidos da relação, disconnect, inserts e comportamento da pivô.
...
Quando `where_complex` é usado, todos os `where_` do builder são ignorados, antes ou depois dele. Veja [Where complex](/v1.2.0/orm/where-complex).
...
- Dentro de transactions, não está disponível no MySQL v1.2.0.
