...
Consulte [Busca full-text](/v1.1.1/orm/full-text-search) para a API `.fulltext(...)` em todos os finds.
...
O planner gera `CREATE INDEX`, `DROP INDEX` ou a variante full-text do adapter. A introspecção compara nome, colunas, ordem e tipo do índice. Snapshots criados antes da v1.1.1 continuam compatíveis: a ausência histórica da propriedade `indexes` não é tratada como remoção intencional.
