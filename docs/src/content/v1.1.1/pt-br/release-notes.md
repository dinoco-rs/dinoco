...
O Dinoco v1.1.1 adiciona workspaces nomeados de banco e migrations SQLite opcionais em runtime. Ele também torna os enums gerados a partir do schema serializáveis com Serde e mantém as melhorias de transactions, índices e query builders da v1.0.9.
...
O comportamento da v1.1.1 é coberto por testes de parser, migrations, query builder, adapters e documentação. A suíte de integração com bancos exercita SQLite, PostgreSQL e MySQL; o PgBouncer usa o mesmo compiler PostgreSQL e o mesmo fluxo de execução transacional.
