...
Consulte [Índices e constraints](/v1.2.7/guide/indexes) para índices simples e compostos, unicidade e índices automáticos de primary e foreign keys.
...
O field virtual não é uma coluna de `tag` e sempre volta como `None`. Para endpoints existentes, use as operações de update `connect` e `disconnect` no mesmo field gerado. Consulte [Many-to-many implícito](/v1.2.7/guide/relations#many-to-many-implícito) para o contrato completo.
