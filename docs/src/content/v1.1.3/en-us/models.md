...
See [Indexes and constraints](/v1.1.3/guide/indexes) for single-field and composite indexes, uniqueness, and automatic primary- and foreign-key indexes.
...
The virtual field is not a column of `tag` and always comes back as `None`. On existing endpoints, use the same generated field's `connect` and `disconnect` update operations. See [Implicit many-to-many](/v1.1.3/guide/relations#implicit-many-to-many) for the complete contract.
