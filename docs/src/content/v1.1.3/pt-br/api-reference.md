...
Para endpoints existentes, use `.connect(value)` e `.disconnect(value)` em `update`, `update_many` ou `find_and_update`. Fields virtuais ficam fora das listas de colunas de `INSERT` e `SELECT` do endpoint e permanecem `None` nas entities retornadas. As duas formas funcionam em batches transacionais; inserts transacionais por chave virtual exigem um ID de endpoint conhecido antes da execução e, por isso, rejeitam uma chave preenchida quando o ID do endpoint usa `autoincrement()`.

## Transactions

`Transaction::new()` cria uma lista heterogênea de builders. Adicione operações com `push`, execute com `transactions(transaction).execute(&client).await?` e leia cada retorno por índice usando `TransactionResults::get::<T>` ou `take::<T>`. O macro `transaction![...]` oferece a forma compacta. Toda a batch, inclusive os writes da pivô gerados por `connect`, `disconnect`, `insert_into` e `insert_many`, usa uma conexão primary e faz rollback no primeiro erro.
