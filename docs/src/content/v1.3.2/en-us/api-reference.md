...
Scalars: `String`, `Boolean`, `Integer`, `Float`, `DateTime`, `Date`, and `Json`. Add `?` for optional scalar values and `[]` for relation lists. Singular relation navigation fields always require `?`; their local foreign keys may be required or optional.
...
For endpoints that already exist, use `.connect(value)` and `.disconnect(value)` in `update`, `update_many`, or `find_and_update`. Virtual fields are excluded from endpoint `INSERT` and `SELECT` column lists and remain `None` in returned entities. Both forms can execute through the closure transaction context.

## Transactions

`transaction(&client, |tx| async move { ... }).await` opens one native transaction on one physical primary connection. Execute every mutation with `.execute(tx)`. An `Ok(value)` commits and returns the value; any error rolls back. `TransactionError` distinguishes create, update, delete, atomic update, commit, and rollback failures while preserving driver errors.
...
`fulltext` works in `find_first`, `find_many`, `find_and_update`, one/many include builders, and `where_complex` trees. It remains available when `find_and_update` executes with a transaction context. A member of `@@fulltexts` searches the complete declared field group. The method does not exist on other Strings.

Integer and float filter fields: `between`. Integer and float update fields, including optional forms: `increment`, `decrement`, `multiply`, and `divide`.
