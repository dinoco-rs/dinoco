...
The generated enum also implements `Display`, so `.to_string()` returns the exact schema value instead of the PascalCase Rust variant name. `TryFrom<&str>`, `TryFrom<String>`, and `FromStr` provide the reverse conversion and return an error for unknown values. For example, `waiting_payment` becomes `PaymentState::WaitingPayment`; `.to_string()` returns `"waiting_payment"`, and `PaymentState::try_from("waiting_payment")` reconstructs the same variant.
