...
Generated relation keys preserve their wrappers: `Project.owner_id` is `Snowflake`, `Task.project_id` is `Uuid`, and `Task.assignee_id` is `Option<Snowflake>`. The generated `TagTask` pivot correctly mixes `tag_id: Uuid` with `task_id: Snowflake`.
...
let project = Project::new(account.id, "Dinoco 1.0.9".to_string());
...
```rust
let tag = Tag::new("documentation".to_string());
dinoco::insert_into::<Tag>().values(&tag).execute(&client).await?;

dinoco::update::<TagTask>()
    .where_(|pivot| pivot.task_id.eq(task.id))
    .update(|pivot| pivot.tag_id.connect(&tag.id))
    .execute(&client)
    .await?;

let links = dinoco::find_many::<TagTask>()
    .where_(|pivot| pivot.task_id.eq(task.id))
    .execute(&client)
    .await?;

let tag_ids = links.iter().map(|link| &link.tag_id).collect::<Vec<_>>();
let tags = dinoco::find_many::<Tag>()
    .where_(|tag| tag.id.batch(tag_ids))
    .execute(&client)
    .await?;

dinoco::update::<TagTask>()
    .where_(|pivot| pivot.task_id.eq(task.id))
    .update(|pivot| pivot.tag_id.disconnect(&tag.id))
    .execute(&client)
    .await?;
```

`connect` inserts only the pivot row. `disconnect` removes only that row.
...
7. Use the generated pivot entity for many-to-many links.
