...
Fields declared with `@fulltext` also expose `.fulltext(term)`. Regular strings do not have that method. See [Full-text search](/v1.0.9/orm/full-text-search).

## Numeric ranges

Integer, float, and optional numeric fields support inclusive ranges:
...
For precedence with `AND`, `OR`, and `NOT`, see the dedicated [Where complex](/v1.0.9/orm/where-complex) page.
