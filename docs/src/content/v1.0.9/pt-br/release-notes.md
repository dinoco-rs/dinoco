...
O Dinoco v1.0.9 é uma atualização de compatibilidade que torna os futures das batches transacionais seguros para runtimes async multithread e handlers do Axum. Ela inclui os índices de schema, batches transacionais atômicas e filtros booleanos com agrupamento explícito introduzidos na v1.0.8.
...
O comportamento da v1.0.9 é coberto por testes de parser, migrations, query builder, adapters e documentação. A suíte de integração com bancos exercita SQLite, PostgreSQL e MySQL; o PgBouncer usa o mesmo compiler PostgreSQL e o mesmo fluxo de execução transacional.
