...
Consulte [Índices e constraints](/v1.3.3/guide/indexes) para índices simples e compostos, unicidade e índices automáticos de primary e foreign keys.
...
O field virtual não é uma coluna de `tag` e sempre volta como `None`. Para endpoints existentes, use as operações de update `connect` e `disconnect` no mesmo field gerado. Consulte [Many-to-many implícito](/v1.3.3/guide/relations#many-to-many-implícito) para o contrato completo.
...
O módulo raiz gerado começa com `#![allow(unused)]`, suprimindo warnings do módulo gerado e dos arquivos que ele importa. Derives adicionais e seus imports devem ser configurados em `config.custom_derives`; consulte [Organização do schema](/v1.3.3/guide/schema-organization#custom-derives).
