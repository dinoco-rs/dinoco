...
Consulte [Organização do schema](/v1.3.3/guide/schema-organization) para estruturas de projeto, imports nomeados, regras de escopo, validação de caminhos e um exemplo multi-arquivo completo.
...
Cada objeto exige `into`, `derive` e `import`. O alvo deve ser `"enum"` ou `"struct"`, o derive deve ser um caminho Rust válido e o import deve ser uma declaração `use ...` em uma única linha. Mantenha `custom_derives` no nível principal de `config`, fora dos workspaces. As crates que fornecem as macros continuam sendo dependências da aplicação. Consulte [Organização do schema](/v1.3.3/guide/schema-organization#custom-derives) para a saída gerada e todas as regras de validação.
