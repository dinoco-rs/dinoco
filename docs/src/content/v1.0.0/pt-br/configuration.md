...
config {
    ...
    read_réplicas = [env("DATABASE_REPLICA_1")]
}
```
...
`database_url`, cada item de `read_réplicas` e `snowflake_node_id` aceitam somente `env("NOME")`. O compiler rejeita valores literais.
...
O `database_url` é resolvido pela CLI e pelo `connect()` gerado. Para configurar réplicas no runtime, construa os adapters declarados e use `with_read_réplicas`.
