...
Vários `where_` são combinados com `AND`. Use [Where complex](/v1.3.0/orm/where-complex) quando a precedência precisar de grupos `AND`, `OR` e `NOT`.
...
Consulte [Busca full-text](/v1.3.0/orm/full-text-search) para as diferenças entre adapters.
...
O retorno passa a ser `Vec<AccountSummary>`. `includes(...)` pode carregar relações, filtrar children e aplicar paginação por parent. Veja [Select](/v1.3.0/orm/select) e [Includes](/v1.3.0/orm/includes).
