...
`find_first` and `find_many` execute against `&client` or against the `tx` of a transaction closure; the other read builders take `&client` only.
...
`insert_into` and `insert_many` fail with `CreateError`, which has one variant per constraint kind (`UniqueViolation`, `ForeignKeyViolation`, `NotNullViolation`, `CheckViolation`). See [Handle insert errors](/en-us/docs/orm/orm/insert#handle-insert-errors).
