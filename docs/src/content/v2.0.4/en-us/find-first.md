...
`pluck` projects onto one column and returns its raw value directly, instead of a row-model struct:
...
It replaces `select`/`includes` rather than combining with them — `PluckValue` decodes the column straight from the row.

## Transform the result

`transform` runs a plain Rust closure over the row, if one was found, after the query (and any `.includes(...)`) has already run:
...
Every `.includes(...)` on that same query follows it to the primary too — you can't route the main row to the primary while letting its includes fall back to a replica.

## Inside a transaction

`execute` also accepts the `tx` handed to a [`transaction(...)`](/en-us/docs/orm/orm/transactions#read-inside-a-transaction) closure. The query — and every `.includes(...)` it loads — then runs on the transaction's own connection, so it sees rows the closure inserted or updated before committing:
...
dinoco::transaction(&client, |tx| async move {
    ...
    let stored = dinoco::find_first::<Account>()
        ...
        .execute(tx)
        .await?; // Some(account), even though nothing is committed yet
...
`.pluck(...)` and `.transform(...)` work the same way. `read_in_primary()` has nothing to change there: a transaction always runs on the primary.
