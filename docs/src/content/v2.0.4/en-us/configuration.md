...
`migration_engine` chooses who writes the database changes. It is optional and defaults to `"automatic"`:
...
| Value | Behavior |
| --- | --- |
| `"automatic"` (default) | Dinoco compares `schema.dinoco` with the database and generates `up.sql`/`down.sql` migrations |
| `"manual"` | You write each migration in Rust, in `dinoco/migrations/`, and `dinoco migrate generate <name>` only scaffolds it |

Any other value is a compile error. The setting is per database, so with `workspace` blocks each workspace may choose its own engine. See [Manual migrations](/en-us/docs/orm/guide/manual-migrations) for the full guide.

> [!NOTE]
> `custom_derives` was removed from `config`. To add derives, attributes, imports, or methods to generated code, use [code transforms](/en-us/docs/orm/guide/transforms) — a schema that still declares `custom_derives` fails to compile with a pointer there.
...
### Saved schema per workspace

Each time `migrate generate` or `models generate` runs for a workspace, Dinoco also saves the schema it used into that workspace's folder, next to its migrations. Every file is copied: `schema.dinoco` and everything it imports, directly or through other files, with the same folder layout:

```text
dinoco/
  schema.dinoco              <- the schema you edit
  enums.dinoco
  domain/business.dinoco
  migrations/
    dev/
      1721320123456_generated/
      schema/                <- dev's saved copy
        schema.dinoco
        enums.dinoco
        domain/business.dinoco
    prod/
      1721320999999_generated/
      schema/                <- prod's saved copy
        ...
```

- The copy shows the schema each workspace was last generated from. After switching from `dev` to `prod` and editing the schema, `migrations/dev/schema/` still holds what `dev` was generated with.
- Only the selected workspace's copy is written, and it is replaced as a whole: a file the schema stops importing disappears from the next copy. `.dinoco` files in `dinoco/` that the schema doesn't load aren't copied.
- Your files in `dinoco/` are never changed. The copy is only a record, so it's worth committing it together with the migrations.
- Files keep their paths relative to `dinoco/`, so the copy compiles on its own. A file imported from outside `dinoco/` (for example `../shared/common.dinoco`) is saved under `schema/_external/`, with each `..` written as `_up`: `schema/_external/_up/shared/common.dinoco`.
- The `schema/` folder is never read as a migration. Projects without `workspace` don't get a copy, because `dinoco/` already holds their only schema.
...
## find_batch execution mode

`query_mode` controls how `find_batch(...)` runs the items passed to it (see [Query overview](/en-us/docs/orm/orm/find#batch-several-queries-together)). It defaults to `"batch_query"` and can be set either at the top level or inside an individual workspace:
...
- `"batch_query"` (default) — every item in a `find_batch(...)` call runs as its own query, same as calling `.execute(...)` on each of them individually.
- `"single_query"` — every item is compiled into a JSON-aggregated subquery and selected together in a single round trip.

This only changes generated code's default (`dinoco::QueryMode` passed to `.with_query_mode(...)` on the generated client); it can still be overridden at runtime by calling `.with_query_mode(...)` again on the client.
