...
`read_in_primary()` routes this query — and every relation it `.includes(...)` — away from replicas and onto the primary. Reach for it specifically when the read depends on a write that just happened and can't tolerate replication lag.

## Inside a transaction

Pass the `tx` of a [`transaction(...)`](/en-us/docs/orm/orm/transactions#read-inside-a-transaction) closure instead of `&client` to read on the transaction's connection. The rows reflect every write the closure has made so far, and `.includes(...)`, `.pluck(...)`, and `.transform(...)` run there too:
