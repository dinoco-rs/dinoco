...
`migration_engine` escolhe quem escreve as mudanças no banco. É opcional e assume `"automatic"`:
...
| Valor | Comportamento |
| --- | --- |
| `"automatic"` (padrão) | O Dinoco compara o `schema.dinoco` com o banco e gera migrations `up.sql`/`down.sql` |
| `"manual"` | Você escreve cada migration em Rust, em `dinoco/migrations/`, e `dinoco migrate generate <nome>` só cria o esqueleto |

Qualquer outro valor é um erro de compilação. A configuração é por banco, então com blocos `workspace` cada workspace pode escolher o seu motor. Veja [Migrations manuais](/pt-br/docs/orm/guide/manual-migrations) para o guia completo.

> [!NOTE]
> O `custom_derives` foi removido do `config`. Para adicionar derives, atributos, imports ou métodos ao código gerado, use os [code transforms](/pt-br/docs/orm/guide/transforms) — um schema que ainda declara `custom_derives` falha ao compilar com um aviso apontando para lá.
...
### Schema salvo por workspace

Toda vez que `migrate generate` ou `models generate` roda para um workspace, o Dinoco também salva o schema usado na pasta daquele workspace, ao lado das migrations. Todos os arquivos são copiados: o `schema.dinoco` e tudo o que ele importa, direta ou indiretamente, com a mesma estrutura de pastas:

```text
dinoco/
  schema.dinoco              <- o schema que você edita
  enums.dinoco
  domain/business.dinoco
  migrations/
    dev/
      1721320123456_generated/
      schema/                <- cópia salva do dev
        schema.dinoco
        enums.dinoco
        domain/business.dinoco
    prod/
      1721320999999_generated/
      schema/                <- cópia salva do prod
        ...
```

- A cópia mostra o schema a partir do qual cada workspace foi gerado pela última vez. Depois de trocar de `dev` para `prod` e editar o schema, `migrations/dev/schema/` continua com o que o `dev` usou.
- Só a cópia do workspace selecionado é escrita, e ela é substituída por inteiro: um arquivo que o schema deixa de importar some da próxima cópia. Arquivos `.dinoco` em `dinoco/` que o schema não carrega não são copiados.
- Os seus arquivos em `dinoco/` nunca são alterados. A cópia é só um registro, então vale commitá-la junto com as migrations.
- Os arquivos mantêm os caminhos relativos a `dinoco/`, então a cópia compila sozinha. Um arquivo importado de fora de `dinoco/` (por exemplo `../shared/common.dinoco`) é salvo em `schema/_external/`, com cada `..` escrito como `_up`: `schema/_external/_up/shared/common.dinoco`.
- A pasta `schema/` nunca é lida como migration. Projetos sem `workspace` não recebem cópia, porque o `dinoco/` já guarda o único schema deles.
...
## Modo de execução do find_batch

`query_mode` controla como `find_batch(...)` roda os itens passados para ele (veja [Visão geral de queries](/pt-br/docs/orm/orm/find#rodando-várias-queries-juntas)). O default é `"batch_query"`, e pode ser definido tanto no nível principal quanto dentro de um workspace individual:
...
- `"batch_query"` (default) — cada item de uma chamada `find_batch(...)` roda como sua própria query, igual a chamar `.execute(...)` em cada um individualmente.
- `"single_query"` — cada item é compilado em uma subquery agregada em JSON e todos são selecionados juntos em um único round trip.

Isso só muda o default do código gerado (o `dinoco::QueryMode` passado para `.with_query_mode(...)` no client gerado); ainda pode ser sobrescrito em runtime chamando `.with_query_mode(...)` de novo no client.
