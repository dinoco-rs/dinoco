...
`find_first` e `find_many` executam contra `&client` ou contra o `tx` de uma closure de transaction; os outros builders de leitura aceitam apenas `&client`.
...
`insert_into` e `insert_many` falham com `CreateError`, que tem uma variante por tipo de constraint (`UniqueViolation`, `ForeignKeyViolation`, `NotNullViolation`, `CheckViolation`). Veja [Trate erros de insert](/pt-br/docs/orm/orm/insert#trate-erros-de-insert).
