...
No schema, UUID continua sendo declarado como `String` e Snowflake como `Integer`. O codegen da v1.2.4 acompanha a chave referenciada e preserva o wrapper Rust:
...
`insert_into` e `insert_many` com IDs virtuais preenchidos também podem executar com `.execute(tx)` dentro da API transacional baseada em closure, tornando atômicas as rows do endpoint e da pivô. Veja [Transações](/v1.2.4/orm/transactions#builders-suportados) para o fluxo de mutations suportado.
