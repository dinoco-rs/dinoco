...
async fn exercise_crud_flow<A: DinocoAdapter>(client: &DinocoClient<A>) -> DinocoResult<()> {
    ...
    assert_eq!(inserted_many.iter().map(|item| item.id).collect::<Vec<_>>(), vec![2, 3]);
    assert_eq!(inserted_many[0].email, "ana@dinoco.dev");
    assert_eq!(inserted_many[1].note.as_deref(), Some("legacy"));
    ...
    assert_eq!(updated_single.len(), 1);
