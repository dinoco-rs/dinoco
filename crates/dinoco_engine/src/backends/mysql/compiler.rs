    ...
    fn compile_find_batch_query(&self, query: FindBatchQuery) -> (String, Vec<DinocoValue>) {
        ...
        for (index, item) in query.items.into_iter().enumerate() {
            ...
            expressions.push(format!(
                "(SELECT COALESCE(JSON_ARRAYAGG(JSON_OBJECT({object_fields})), JSON_ARRAY()) FROM ({inner}) AS __dinoco_batch_{index}) AS c{index}"
            ));
        }
...
fn compile_relation_occurrence_query(query: RelationOccurrenceQuery) -> (String, Vec<DinocoValue>) {
    ...
    let parent_keys = (0..query.key_count)
        .map(|ordinal| {
            if ordinal == 0 {
                "SELECT 0 AS __dinoco_relation_ordinal, ? AS __dinoco_relation_key".to_string()
            } else {
    ...
    let mut sql = format!(
        "WITH __dinoco_parent_keys AS ({parent_keys}) SELECT {} FROM __dinoco_parent_keys INNER JOIN {} ON __dinoco_parent_keys.__dinoco_relation_key = {}.{}",
        fields.join(", "),
        query.query.from,
        query.query.from,
        query.child_field,
