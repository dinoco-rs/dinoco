...
- `.returning()`: retorna os models inseridos.
- `.returning_as::<T>()`: retorna os itens inseridos em uma projeção tipada.
...
Com `.returning()`, o retorno é `DinocoResult<Vec<User>>`. Com `.returning_as::<T>()`, o retorno passa a ser:
