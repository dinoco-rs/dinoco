...
- `.values(Vec<M>)` ou `.values(Vec<&M>)`: define os registros que serão inseridos em lote.
...
```rust
let user_a = User { id: "u3".into(), email: "c@acme.com".into(), name: "C".into() };
let user_b = User { id: "u4".into(), email: "d@acme.com".into(), name: "D".into() };
...
- [**Derives**](/v0.0.6/core/derives): entenda como montar payloads com `Extend` e `#[insertable]`.
- [**`insert_into::<M>()`**](/v0.0.6/orm/insert-into): inserção única.
- [**`update::<M>()`**](/v0.0.6/orm/update): atualização com filtro.
