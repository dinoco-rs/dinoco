...
Enums allow restricting a field to a fixed set of known values in the Dinoco schema.
...
- `@default(USER)` defines the default value for the field.

## When to use enums

Enums are useful for representing values like:
...
- Use `@default(...)` when there's a natural initial state.
