...
`モデル`は、Dinocoスキーマにおけるアプリケーションの中心的なエンティティを定義します。各`モデル`は通常、データベース内のテーブルを表し、コード生成、型付きクエリ、およびDinoco APIでの操作の基盤として機能します。
...
- これらのデータがcodegenとAPIによってどのように使用されるか。
...
これらは、テキスト、数値、ブール値、日付などの直接的な値を保存するフィールドです。
...
この形式は、データベースとワークフローがこの種の構造をサポートしている場合に、値のリストを表します。
...
生成されたAPIが安全に動作するためには、各モデルに明確な識別子が必要です。
...
| `@default(now())`           | 現在の日付            |
| `@default(autoincrement())` | 自動インクリメント整数 |
...
| :-------------------- | :---------------------------------------- |
| `@@ids([...])`        | 複合プライマリキーを定義します            |
| `@@table_name("...")` | データベース内の実際のテーブル名にマッピングします |
...
スキーマ内でよりユーザーフレンドリーなモデル名を維持しつつ、データベース内の別の物理名にマッピングしたい場合に`@@table_name()`を使用します。
...
- スキーマおよび生成されたAPIでは、モデルは引き続き`User`と呼ばれます。
...
単一フィールドでのアトミックな更新が必要な場合は、`find_and_update`ワークフローの方がより直接的です。
...
| スカラーフィールド | `email String`           | 単純な値を保存する         |
| オプションフィールド | `name String?`           | 値の欠落を許可する         |
| リストフィールド | `tags String[]`          | 複数の値を保存する         |
| ID              | `id Integer @id`         | 一意に識別する             |
| デフォルト値    | `@default(now())`        | 自動的に入力する           |
| 一意            | `email String @unique`   | 重複を避ける               |

## 新しいモデルを作成するタイミング

アプリケーションのエンティティが以下を必要とする場合、通常、新しい`モデル`を作成します。
