export const title = "count";

# count

Используется для подсчета записей.

---

## Что можно сделать

- Фильтрация с помощью `.cond(...)`
- Выполнение с помощью `.execute(&client)`

## Описание методов

- `.cond(...)`: ограничивает, какие записи будут включены в подсчет.
- `.execute(&client)`: выполняет подсчет в базе данных.

## Возвращаемое значение

Возвращаемое значение `count` это:

```rust
DinocoResult<usize>
```

## Базовый пример

```rust
let total = dinoco::count::<User>()
    .execute(&client)
    .await?;
```

## Пример с логическим фильтром

```rust
let total = dinoco::count::<User>()
    .cond(|w| w.active.eq(true))
    .execute(&client)
    .await?;
```

## Пример с текстовым фильтром

```rust
let total = dinoco::count::<User>()
    .cond(|w| w.name.includes("Ana"))
    .execute(&client)
    .await?;
```

## Следующие шаги

- [**`find_many::<M>()`**](/v0.0.1/orm/find-many): извлекает записи в виде списка.
- [**`find_first::<M>()`**](/v0.0.1/orm/find-first): извлекает одну запись.
