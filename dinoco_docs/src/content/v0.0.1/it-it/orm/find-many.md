...
Utilizzato per recuperare un elenco di record.

---

## Cosa puoi fare

- Selezionare una proiezione con `.select::&lt;T&gt;()`
- Filtrare con `.cond(...)`
- Limitare con `.take(...)`
- Paginare con `.skip(...)`
- Ordinare con `.order_by(...)`
- Caricare relazioni con `.includes(...)`
- Contare relazioni con `.count(...)`
- Forzare la lettura sul database primario con `.read_in_primary()`
- Eseguire con `.execute(&client)`

## Descrizione dei metodi

- `.select::&lt;T&gt;()`: sostituisce la proiezione predefinita del modello con una proiezione personalizzata.
...
- `.read_in_primary()`: forza la lettura sul database primario, senza usare repliche.
- `.execute(&client)`: esegue la query sul database.
...
## Esempio di select personalizzata
...
## Esempio di lettura sul database primario
...
- [**`find_first::&lt;M&gt;()`**](/v0.0.1/orm/find-first): versione per cercare al massimo un record.
- [**`count::&lt;M&gt;()`**](/v0.0.1/orm/count): conteggio di record con filtro.
