export const title = "Configuração";
...
- Com `redis`, o projeto passa a expor helpers como `client.cache()`, `.cache(...)` e filas com `workers()`.
- Com `read_replicas`, leituras podem usar réplica enquanto writes continuam no banco principal.
