...
- montar cache manual
- invalidar chaves depois de writes
- compartilhar payloads entre várias queries
...
- `client.cache()` é cache manual; ele não executa query no banco.
- Para cache integrado em query, use `find_first().cache(...)` e `find_many().cache(...)`.
