export const title = 'Derivazioni';

# Derivazioni

Panoramica delle derivazioni e degli attributi centrali utilizzati in Dinoco.

---

## Cosa sono le derivazioni in Dinoco

Le derivazioni di Dinoco aiutano a collegare le tue struct Rust al comportamento dell'ORM senza dover scrivere manualmente l'intera implementazione.

Le più comuni nell'uso quotidiano sono:
...
Questa derivazione esegue la serializzazione della riga restituita dall'adapter nella struct Rust.
...
In questo modo, Dinoco inserisce il padre e poi elabora automaticamente gli elementi annidati.
...
- Usa `Rowable` per il modello principale generato o per le struct compatibili con la lettura diretta delle righe.
- Usa `Extend` per proiezioni, conteggi, inclusioni e payload specializzati.
- Usa `#[insertable]` quando `Extend` deve servire anche come payload di scrittura con annidamento.

## Prossimi passi

- [**Traits**](/v0.0.2/core/traits): vedi le trait implementate dal modello.
