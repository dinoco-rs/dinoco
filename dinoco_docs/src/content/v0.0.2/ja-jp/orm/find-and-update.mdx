...
単一のレコードを見つけ、データベースにアトミックな更新を適用し、更新された要素を返すために使用されます。
...
- 条件に一致する行がない場合、`DinocoError::RecordNotFound` が返されます。
- 更新DSLはリレーションを公開しません。
- 現在、ワークフローは、更新されたレコードを見つけて返すために単純な主キーをサポートしています。
