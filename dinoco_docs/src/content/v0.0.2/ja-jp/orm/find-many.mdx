...
レコードのリストをフェッチするために使用されます。

---

## できること

- `.select::<T>()`: モデルのデフォルトのプロジェクションをカスタムプロジェクションに切り替えます。
...
- `.cache(...)`: 指定されたキーを使用して最初にRedisをクエリします。キャッシュミスの場合、クエリを実行し、結果を保存して返します。キャッシュヒットの場合、クエリロガーは`CACHE HIT key=...`を記録します。
- `.cache_with_expiration(...)`: 標準キャッシュと同じ動作ですが、TTLを秒単位で書き込みます。
...
- [**`find_first::<M>()`**](/v0.0.2/orm/find-first): 最大1つのレコードを検索するためのバージョン。
- [**`count::<M>()`**](/v0.0.2/orm/count): フィルター付きのレコード数。
