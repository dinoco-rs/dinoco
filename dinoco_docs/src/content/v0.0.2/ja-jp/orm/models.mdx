...
`モデル`は、Dinocoスキーマにおけるアプリケーションの中心的なエンティティを定義します。各`モデル`は通常、データベース内のテーブルを表し、コード生成、型付きクエリ、およびDinoco APIとの操作の基盤となります。
...
テキスト、数値、ブール値、日付などの直接的な値を格納するフィールドです。
...
| :-------------------- | :------------------------------------ |
| `@@ids([...])`        | 複合主キーを定義します                 |
| `@@table_name("...")` | データベース内の実際のテーブル名をマッピングします |
...
- モデルはスキーマと生成されたAPIで`User`という名前のままです。
...
単一フィールドでのアトミックな更新が必要な場合、`find_and_update`フローはさらに直接的です。
...
アプリケーションのエンティティが以下のいずれかを必要とする場合、通常、新しい`モデル`を作成します。
