...
`--apply`オプションでマイグレーションを生成すると、データベースに適用され、モデルが自動的に生成されます！
...
async fn main() -> dinoco::DinocoResult<()> {
    ...
    insert_into::<User>()
        .values(UserWithPostInsert {
            ...
            posts: vec![Post { id: 0, title: "最初の投稿".to_string(), published: true, authorId: None }],
        })
        .execute(&client)
        .await?;
...
- [**Dinocoスキーマ**](/v0.0.2/orm/introduction-dinoco): Dinocoの構造と目的をよりよく理解しましょう。
- [**find_many**](/v0.0.2/orm/find-many): リストクエリでのフィルター、インクルード、キャッシュについて見てみましょう。
