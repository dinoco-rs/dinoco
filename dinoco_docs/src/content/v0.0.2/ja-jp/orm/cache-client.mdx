...
`client.cache()` は、`find_first` および `find_many` に組み込まれたキャッシュヘルパーに依存することなく、`DinocoClient` で設定された Redis への直接アクセスを公開します。
...
- `.get::<T>(key)`: 値を `T` として検索し、デシリアライズします
- `.set(key, &value)`: シリアライズして TTL なしで保存します
- `.set_with_ttl(key, &value, ttl_seconds)`: シリアライズして秒単位の有効期限付きで保存します
- `.delete(key)`: キーを削除します
...
値は JSON でシリアライズされるため、型は `serde` と互換性がある必要があります。
