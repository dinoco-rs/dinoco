...
Dans ce cas, la struct reste liée au modèle `User`, mais ne peut exposer que les champs nécessaires à ce flux.
...
- Utilisez `Extend` pour les projections, les comptages, les includes et les charges utiles spécialisées.
- Utilisez `#[insertable]` lorsque `Extend` doit également servir de charge utile d'écriture avec imbrication.

## Prochaines étapes

- [**Traits**](/v0.0.2/core/traits) : voir les traits implémentés par le modèle.
