...
Aperçu des traits centraux utilisés par les modèles et les payloads dans Dinoco.
...
`Projection` définit comment une structure lit les colonnes d'un modèle et quelles colonnes elle doit charger.
...
Elle est très utilisée avec `select::<T>()`, `include` et les structures dérivées avec `Extend`.
...
`UpdateModel` décrit les champs qui peuvent être envoyés lors d'une mise à jour et comment identifier l'enregistrement à modifier.
...
- contenu imbriqué qui sera traité par la suite

C'est ce trait que `#[insertable]` implémente automatiquement.

## Trait InsertConnectionPayload

`InsertConnectionPayload` est utilisé lorsque le payload imbriqué représente des connexions avec des enregistrements existants.
...
Dans un modèle généré par Dinoco, il est courant de recevoir déjà des implémentations de :
...
- des traits comme `Model`, `Projection` et `InsertModel` générés automatiquement par le codegen
...
- Utilisez `Projection` lorsque la structure doit être chargée par `select`.
