...
`client.cache()` expose un accès direct à Redis configuré dans le `DinocoClient`, sans dépendre des helpers de cache couplés à `find_first` et `find_many`.
...
Utilisez `client.cache()` lorsque vous voulez :

- monter un cache manuel
- invalider des clés après des écritures
- partager des payloads entre plusieurs requêtes
...
- `client.cache()` est un cache manuel ; il n'exécute pas de requête sur la base de données.
