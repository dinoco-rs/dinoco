...
- **Models :** Les représentations typées pour une utilisation dans votre code Rust.
...
async fn main() -> dinoco::DinocoResult<()> {
    ...
    insert_into::<User>()
        .values(UserWithPostInsert {
            ...
            posts: vec![Post { id: 0, title: "Mon premier post".to_string(), published: true, authorId: None }],
        })
        .execute(&client)
        .await?;
...
- [**find_many**](/v0.0.2/orm/find-many) : Voir les filtres, les inclusions et le cache dans les requêtes de liste.
