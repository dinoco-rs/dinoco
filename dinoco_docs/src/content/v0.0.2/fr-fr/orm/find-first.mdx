...
- `.select::<T>()`: remplace la projection par défaut par une projection personnalisée.
...
- `.cache(...)`: tente de récupérer d'abord dans Redis et ne consulte la base de données que si la clé n'existe pas. En cas de succès du cache, le journal de requête enregistre `CACHE HIT key=...`.
