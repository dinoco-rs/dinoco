...
| :----------- | :----------------------------------------------- |
| `Cascade`    | Propage l'opération aux enregistrements dépendants |
...
- Un `Role` peut appartenir à plusieurs `User`.
...
Pour insérer un enregistrement avec une relation 1:1 en utilisant la nouvelle API, placez la relation à l'intérieur d'un payload `Extend` marqué avec `#[insertable]`.
...
- L'`User` est inséré en premier.
- Le `Profile` lié est créé ensuite.
- La clé `userId` est liée par le flux de relation de Dinoco.

## Exemple d'insert_many avec relations imbriquées

Utilisez `#[insertable]` lorsque vous souhaitez insérer plusieurs enregistrements parents et, pour chacun d'eux, insérer également plusieurs nouveaux enregistrements liés.
...
- Dinoco crée les `Comment` imbriqués et lie chaque groupe au `Post` correct lors de l'écriture.

## Exemple d'insert_many avec connexions imbriquées
...
- Les `Tag` déjà existantes sont connectées aux articles correspondants.
- Chaque payload définit ses connexions en utilisant l'énumération générée par le codegen.
