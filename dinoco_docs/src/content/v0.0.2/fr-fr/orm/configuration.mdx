...
- `read_replicas` : optionnel, ajoute des répliques de lecture.
- `redis` : optionnel, active le cache et les files d'attente basés sur Redis.
...
- Avec `redis`, le projet expose des helpers comme `client.cache()`, `.cache(...)` et des files d'attente avec `workers()`.
