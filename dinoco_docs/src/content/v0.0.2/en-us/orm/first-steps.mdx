...
async fn main() -> dinoco::DinocoResult<()> {
    ...
    insert_into::<User>()
        .values(UserWithPostInsert {
            ...
            posts: vec![Post { id: 0, title: "My first post".to_string(), published: true, authorId: None }],
        })
        .execute(&client)
        .await?;
...
-   [**find_many**](/v0.0.2/orm/find-many): See filters, includes, and caching in list queries.
