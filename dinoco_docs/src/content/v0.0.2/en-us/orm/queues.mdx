...
Dinoco's queue system allows publishing asynchronous events from `insert`, `find`, and `update`, storing in `Redis` only the necessary data to re-hydrate the record when the worker executes.

## What you can do

- Allow chaining .enqueue(...), .enqueue_in(...) and .enqueue_at(...) on all client methods, except for delete operations.
...
- `.enqueue_at(event, date_utc)`: schedules for a specific date in `chrono::DateTime<Utc>`.
...
When calling `run`, Dinoco creates a new `DinocoClient` exclusive to the workers, with its own connection pool. This way, the queue loop does not reuse the same pool as the main application.
...
let _worker = workers()
    .on::<User, _, _>("user.created", |job| async move {
        println!("Processing {}", job.data.name);
        job.success()
    })
    .run()
    .await?;
