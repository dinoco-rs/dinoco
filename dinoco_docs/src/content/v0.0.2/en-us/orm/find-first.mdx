...
- `.select::<T>()`: changes the default projection to a custom projection.
...
- `.includes(...)`: loads relationships along with the main item.
- `.count(...)`: calculates relationship counters in the projection.
- `.cache(...)`: tries to fetch from Redis first and only queries the database if the key does not exist. On a cache hit, the query logger records `CACHE HIT key=...`.
- `.cache_with_expiration(...)`: performs the same flow, but saves with TTL in seconds.
