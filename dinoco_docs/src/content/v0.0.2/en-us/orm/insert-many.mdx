...
This is a common use in many-to-many relationships.
...
`insert_many` can also receive richer payloads through `.values(...)` when the struct is marked with `#[insertable]`. This is useful for creating the parent rows and their nested relations in one recursive write flow.
...
- `with_connection(...)` and `with_connections(...)` are focused on connecting existing relations.
- `#[insertable]` on an `Extend` struct lets `.values(...)` recursively create nested relations without calling `with_relation(...)`.
