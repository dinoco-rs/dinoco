...
- `.values(Vec<M>):` определяет записи, которые будут вставлены пакетом.
...
Когда связанные элементы уже существуют, используйте перечисление, сгенерированное codegen, внутри полезной нагрузки.
...
`insert_many` также может принимать более сложные полезные нагрузки через `.values(...)`, когда структура помечена `#[insertable]`. Это полезно для создания родителей и их вложенных отношений в одном рекурсивном потоке.
