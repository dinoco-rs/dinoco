...
- `.cache_with_expiration(...)`: то же поведение, что и у стандартного кэша, но сохраняет с TTL в секундах.
...
## Пример с простым включением связанных сущностей
...
## Пример с фильтрованным включением связанных сущностей
...
## Пример с вложенным включением связанных сущностей
...
Этот метод генерируется только тогда, когда схема `config {}` содержит `redis`.
...
## Пример с кэшированием и сроком действия
...
- [**`find_first::<M>()`**](/v0.0.2/orm/find-first): версия для получения максимум одной записи.
