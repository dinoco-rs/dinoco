...
要开始使用 Dinoco，您需要安装我们的 CLI，以便能够操作迁移和其他系统！

```bash
cargo install dinoco-cli
```

要创建 Dinoco 环境，请执行以下命令：
...
- **Migrations (迁移):** 您的数据库更改历史记录。
- **Schema (模式):** 您的数据结构的核心定义。
- **Models (模型):** 用于 Rust 代码中的类型化表示。

## 工作原理？

### 1. 定义您的模式和连接

Dinoco 模式定义了您的模型内容和数据库配置。
...
- [**Dinoco 模式**](/v0.0.2/orm/introduction-dinoco): 更好地理解 Dinoco 的结构和目的。
