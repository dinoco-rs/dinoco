...
## Extend #[insertable] 示例

`insert_many` 也可以通过 `.values(...)` 接收更丰富的 payload，当结构体被标记为 `#[insertable]` 时。这对于在一个递归流中创建父级及其嵌套关系非常有用。
...
- 在 `Extend` 结构体上使用 `#[insertable]` 允许 `.values(...)` 以递归方式插入新的关系。
- 对于与现有记录的连接，请使用 codegen 生成的 `ModelConnection` 枚举。
- 新的关系可以使用 `Vec<ModelRelacionado>` 或其他标记为 `#[insertable]` 的 `Extend` payload。
- 现有连接在多对多场景以及由生成模型支持的流程中运行良好。

## 下一步

- [**Derives**](/v0.0.2/core/derives): 了解如何使用 `Extend` 和 `#[insertable]` 构建 payload。
