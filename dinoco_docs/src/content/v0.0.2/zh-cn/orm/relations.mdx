...
Dinoco 暴露引用操作来控制相关记录被更改或删除时发生的情况。
...
- 当可以在不删除子记录的情况下解除关系时，使用 `onDelete: SetNull`。
- 使用带有显式名称的自引用关系，以方便阅读和代码生成。
...
当您想通过一次面向关系的读取来构建丰富的用户视图时，此模式非常有用。

## 使用 User 和 Profile 进行带关系插入的示例

要使用新 API 插入具有 1:1 关系的记录，请将关系放在标记为 `#[insertable]` 的 `Extend` payload 中。
...
- 首先插入 `User`。
- 接着创建相关的 `Profile`。
- `userId` 键由 Dinoco 的关系流绑定。

## 带有嵌套关系的 insert_many 示例
...
## 带有嵌套连接的 insert_many 示例

当您想插入多个父记录并将每个父记录连接到多个已存在的记录时，请使用由代码生成器生成的 `PostConnection` 枚举。
