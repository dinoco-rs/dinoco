...
Eine Selbstbeziehung tritt auf, wenn ein Model sich auf sich selbst bezieht.
...
- Verwenden Sie Selbstbeziehungen mit expliziten Namen, um die Lesbarkeit und den Codegen zu erleichtern.
...
Um einen Datensatz mit einer 1:1-Beziehung über die neue API einzufügen, platzieren Sie die Beziehung in einem `Extend`-Payload, der mit `#[insertable]` markiert ist.
...
- Der Schlüssel `userId` wird durch den Dinoco-Beziehungsfluss verknüpft.
...
- Dinoco erstellt die verschachtelten `Comment` und verknüpft jede Gruppe während des Schreibvorgangs mit dem korrekten `Post`.
...
- Die bereits vorhandenen `Tag` werden mit den entsprechenden Posts verbunden.
