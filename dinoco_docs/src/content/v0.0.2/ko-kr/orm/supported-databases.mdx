export const title = "개요";

# 개요

**Dinoco**는 **PostgreSQL**, **MySQL**, **SQLite** 데이터베이스를 기본적으로 지원합니다. 저희는 곧 새로운 데이터베이스로 지원을 확장하기 위해 노력하고 있습니다!
...
구성(configuration)은 스키마 파일의 `config` 블록 내에서 이루어집니다.

### SQLite

SQLite를 사용하는 경우, 데이터베이스 파일은 `dinoco` 폴더에 자동으로 생성됩니다.
