...
`@relation(...)` 속성은 두 모델이 서로 연결되는 방식을 정의합니다.
...
| `references` | 대상 모델의 참조된 필드                                 |
| `onDelete`   | 참조된 레코드가 삭제될 때의 동작                        |
| `onUpdate`   | 참조된 값이 업데이트될 때의 동작                        |
...
Dinoco는 연결된 레코드가 변경되거나 제거될 때 발생하는 상황을 제어하기 위한 참조 작업을 제공합니다.

지원되는 값:

| 작업         | 의미                                            |
| :----------- | :---------------------------------------------- |
| `Cascade`    | 작업을 종속 레코드로 전파합니다                 |
| `SetNull`    | 외래 키를 `null`로 설정합니다                   |
| `SetDefault` | 필드에 대해 구성된 기본값을 설정합니다          |
...
- 한 `User`는 여러 `Post`를 가질 수 있습니다.
- 각 `Post`는 단일 `User`에 속합니다.

## 다대다

다대다 관계에서는 양쪽에 목록이 있습니다.
...
- 한 `User`는 여러 `Role`을 가질 수 있습니다.
- 한 `Role`은 여러 `User`에 속할 수 있습니다.

## 자기 관계

자기 관계는 모델이 자신을 참조할 때 발생합니다.
...
- 한 사용자는 관리자를 가질 수 있습니다.
- 한 사용자는 여러 부하 직원을 가질 수 있습니다.

## 실용적인 팁

- 동일한 모델 간에 두 개 이상의 관계가 있는 경우 `name`을 사용하십시오.
- 자식이 부모 없이 의미가 없는 경우 `onDelete: Cascade`를 사용하십시오.
- 자식 레코드를 제거하지 않고 관계를 해제할 수 있는 경우 `onDelete: SetNull`을 사용하십시오.
- 가독성과 코드 생성을 용이하게 하기 위해 명시적인 이름을 가진 자기 관계를 사용하십시오.

## 예제용 참조 스키마

다음 예제는 다음 스키마를 사용합니다:
...
## `User`에서 가능한 모든 관계를 조회하는 예제

`User` 모델에서 시작하여 단일 읽기 쿼리에서 모든 직접 관계를 로드하려면 `select::<T>()`를 여러 `includes(...)`와 결합할 수 있습니다.
...
이 패턴은 단일 관계 지향 읽기 작업에서 포괄적인 사용자 보기를 생성하려는 경우에 유용합니다.

## `User`와 `Profile`을 사용한 관계 포함 insert 예제
...
- `User`가 먼저 삽입됩니다.
- 관련 `Profile`이 이어서 생성됩니다.
- `userId` 키는 Dinoco의 관계 흐름을 통해 연결됩니다.

## `with_relations(...)`를 사용한 `insert_many` 예제

여러 부모 레코드를 삽입하고 각 부모 레코드에 대해 여러 개의 새로운 관련 레코드를 삽입하려는 경우 `with_relations(...)`를 사용하십시오.
...
- Dinoco는 쓰기 작업 중에 `Comment`를 올바른 게시물에 연결합니다.

## `with_connections(...)`를 사용한 `insert_many` 예제

여러 부모 레코드를 삽입하고 각 부모 레코드를 여러 개의 이미 존재하는 레코드와 연결하려는 경우 `with_connections(...)`를 사용하십시오.
...
- `Post`가 삽입됩니다.
- 이미 존재하는 `Tag`가 해당 `Post`에 연결됩니다.
- 외부 벡터의 각 그룹은 동일한 위치에 있는 게시물의 연결을 나타냅니다.

## 다음 단계

- [**Enums**](/v0.0.1/orm/enums): 스키마에서 제어된 값을 나타내는 방법을 알아보십시오.
- [**모델**](/v0.0.1/orm/models): Dinoco API를 사용한 필드 구조 및 쿼리, 삽입, 업데이트, 삭제 예제를 살펴보십시오.
