...
Dinoco는 관련 레코드가 변경되거나 제거될 때 발생하는 일을 제어하기 위한 참조 작업을 노출합니다.
...
- 동일한 모델 간에 여러 관계가 있을 때 `name`을 사용하십시오.
...
이 패턴은 단일 관계 지향 읽기에서 풍부한 사용자 뷰를 구성하려는 경우에 유용합니다.

## User 및 Profile을 사용한 관계 삽입 예시

새 API를 사용하여 1:1 관계를 가진 레코드를 삽입하려면, `#[insertable]`로 표시된 `Extend` 페이로드 내에 관계를 배치하십시오.
...
여러 부모 레코드를 삽입하고 각 부모 레코드를 여러 기존 레코드에 연결하려는 경우 코드 생성기가 생성한 `PostConnection` enum을 사용하십시오.
...
- [**Enum**](/v0.0.2/orm/enums): 스키마에서 제어되는 값을 나타내는 방법을 확인하십시오.
