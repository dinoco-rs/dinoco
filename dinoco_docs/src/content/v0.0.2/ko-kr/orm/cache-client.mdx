export const title = '캐시()';

# 캐시

`client.cache()`는 `find_first` 및 `find_many`에 내장된 캐시 헬퍼에 의존하지 않고 `DinocoClient`에 구성된 Redis에 직접 접근할 수 있도록 합니다.
...
- 수동 캐시를 설정하려는 경우
- 쓰기 후 키를 무효화하려는 경우
- 여러 쿼리 간에 페이로드를 공유하려는 경우
- 빠른 읽기를 위해 준비된 구조를 저장하려는 경우
...
- `.set(key, &value)`: TTL 없이 직렬화하여 저장합니다.
- `.set_with_ttl(key, &value, ttl_seconds)`: 초 단위 만료와 함께 직렬화하여 저장합니다.
- `.delete(key)`: 키를 제거합니다.
