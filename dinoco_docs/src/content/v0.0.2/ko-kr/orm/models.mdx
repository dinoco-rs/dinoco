...
`모델`은 Dinoco 스키마에서 애플리케이션의 핵심 엔티티를 정의합니다. 각 `모델`은 일반적으로 데이터베이스의 테이블을 나타내며 코드 생성, 유형화된 쿼리 및 Dinoco API와의 작업의 기반이 됩니다.
...
개별 필드의 속성 외에도 Dinoco는 모델 블록 전체에 적용되는 데코레이터도 지원합니다.
...
- 모델은 스키마와 생성된 API에서 계속 `User`라고 불립니다.
...
애플리케이션의 엔티티가 다음을 필요로 할 때 일반적으로 새 `모델`을 생성합니다:

- 데이터베이스에 영구적으로 저장되어야 합니다.
- 자체 ID를 가져야 합니다.
- 독립적으로 쿼리되어야 합니다.
- 자체 읽기 및 쓰기 규칙을 가져야 합니다.
