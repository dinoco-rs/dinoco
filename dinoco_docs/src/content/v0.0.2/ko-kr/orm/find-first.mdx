...
- `.includes(...)`: 기본 항목과 함께 관계를 로드합니다.
- `.count(...)`: 프로젝션에서 관계 카운터를 계산합니다.
- `.cache(...)`: Redis에서 먼저 검색을 시도하고 키가 존재하지 않는 경우에만 데이터베이스를 쿼리합니다. 캐시 적중 시 쿼리 로거는 `CACHE HIT key=...`를 기록합니다.
- `.cache_with_expiration(...)`: 동일한 흐름을 수행하지만 TTL(초)과 함께 저장합니다.
...
- [**`count::<M>()`**](/v0.0.2/orm/count): 레코드 수입니다.
