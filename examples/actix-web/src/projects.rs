...
pub async fn list(client: web::Data<AppState>) -> Result<web::Json<Vec<ProjectResponse>>, ApiError> {
    ...
    Ok(web::Json(projects.into_iter().map(ProjectResponse::from).collect()))
}

/// `GET /projects/{id}` — `find_first` for the row plus a `count` for the
/// still-open tasks.
pub async fn get(client: web::Data<AppState>, id: web::Path<String>) -> Result<web::Json<ProjectDetail>, ApiError> {
    ...
    Ok(web::Json(ProjectDetail { project: project.into(), open_tasks: open_tasks.total }))
}
...
pub async fn create(
    ...
    let (project, tasks) = transaction(client.get_ref(), |tx| async move {
...
) -> Result<web::Json<ProjectResponse>, ApiError> {
    let payload = payload.into_inner();
    if payload.is_empty() {
        return Err(ApiError::bad_request("Provide at least one field to update"));
    }

    let mut query = update_query::<Project>().where_(|project| project.id.eq(id.into_inner()));
...
pub async fn delete(client: web::Data<AppState>, id: web::Path<String>) -> Result<HttpResponse, ApiError> {
    ...
    transaction(client.get_ref(), |tx| async move {
